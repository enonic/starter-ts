...
As an example, here is how to add https://market.enonic.com/vendors/enonic/lib-static[lib-static]. First, declare its runtime dependency in `build.gradle` so the library is bundled with your app:
...
at runtime, with no list of libraries to maintain. A target is skipped entirely when it has
no source files.
