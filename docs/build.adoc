...
We need a cross-platform way to check whether there are any files to process before invoking a tool, so here are a couple of helper scripts that do just that. Each globs for `.ts` files in the given directory and skips the step entirely when none are found:

.tsup/build.js
[source, JavaScript]
----
include::{sourcedir}tsup/build.js[]
----

.tsup/check.js
[source, JavaScript]
----
include::{sourcedir}tsup/check.js[]
----
