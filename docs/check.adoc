...
Type checking is performed by TypeScript 7 — the native compiler. It only checks types here: the actual transpilation of both server-side and client-side code is done by tsdown, so the compilation targets (ES2015/CommonJS for the server, ES2023/ESM for the client) are unaffected by the TypeScript version. Note that the rest of the toolchain (Jest via `@swc/jest`, linting via `oxlint`) is chosen to not depend on the classic TypeScript compiler API (`createProgram`, `transpileModule`), which the native compiler no longer exports — only `tsc` and an explicitly unstable `typescript/unstable/*` surface remain.
