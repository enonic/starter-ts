...
TIP: tsdown is the actively maintained successor to `tsup`, which this starter previously used. `tsup` is now deprecated, and migrating to tsdown keeps the build on a supported, faster toolchain.
