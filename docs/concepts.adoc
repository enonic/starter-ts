...
There are currently few JavaScript engines capable of running TypeScript directly, so a build step must `transpile` your TypeScript to JavaScript before it can be deployed to Enonic XP. For server-side code the build also targets a conservative language level (ES5), because XP's default engine supports an older version of JavaScript. Client-side assets run in the browser and are built to modern JavaScript (ES2023) instead.
